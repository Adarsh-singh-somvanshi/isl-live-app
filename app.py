...
st.title("🖐️ Indian Sign Language – Real Time Translation")
    ...
    def recv(self, frame):
        ...
        if results.left_hand_landmarks:
            self.mp_drawing.draw_landmarks(
                img, results.left_hand_landmarks, self.mp_holistic.HAND_CONNECTIONS
            )

        if results.right_hand_landmarks:
            self.mp_drawing.draw_landmarks(
                img, results.right_hand_landmarks, self.mp_holistic.HAND_CONNECTIONS
            )
        ...
        if len(self.sequence) == 32:
            try:
                ...
                self.last_conf = float(output[idx]) * 100

            except Exception as e:
                self.last_pred = "Error"
                self.last_conf = 0
